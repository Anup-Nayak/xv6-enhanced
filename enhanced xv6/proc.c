...
struct proc_history {
  ...
  uint memory_utilization; // Total memory used by the process
};

// Global array to store process history
#define MAX_HISTORY 100
struct proc_history history[MAX_HISTORY];
int history_count = 0; // Number of entries in the history

// Function to add a process to the history
void
...
// Function to retrieve and print process history, sorted by PID
int
gethistory(void)
{
  // Acquire the process table lock to safely access the history array
  acquire(&ptable.lock);
  ...
  // Sort the history array by PID (ascending order)
...
exit(void)
{
  ...
  // Add the exiting process to the history
