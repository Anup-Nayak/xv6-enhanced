...
main(void)
{
  ...
  while(getcmd(buf, sizeof(buf)) >= 0){
    ...
    if(buf[0] == 'h' && buf[1] == 'i' && buf[2] == 's' && buf[3] == 't' && buf[4] == 'o' && buf[5] == 'r' && buf[6] == 'y' ){
      // Call the gethistory system call to retrieve and print process history
      gethistory();
      continue;
    }

    if(buf[0] == 'b' && buf[1] == 'l' && buf[2] == 'o' && buf[3] == 'c' && buf[4] == 'k' && buf[5] == ' '){
      // Call the block system call
      char * temp = buf+6;
      int id = atoi(temp);
      block(id);
      continue;
    ...
    if (buf[0] == 'c' && buf[1] == 'h' && buf[2] == 'm' && buf[3] == 'o' && buf[4] == 'd' && buf[5] == ' ') {
      ...
      // Find the space separating the filename and mode
      while (*temp != ' ' && *temp != '\0') {
        temp++;
      }
      if (*temp == '\0') {
      ...
      *temp = '\0'; // Null-terminate the filename
      ...
      // Convert the mode string to an integer
      int mode = atoi(temp);

      // Call the chmod system call
