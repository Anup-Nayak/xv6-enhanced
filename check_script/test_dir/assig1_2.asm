 ...
 5a1:	a1 38 0a 00 00       	mov    0xa38,%eax
{
 ...
 619:	03 43 fc             	add    -0x4(%ebx),%eax
  freep = p;
 61c:	89 15 38 0a 00 00    	mov    %edx,0xa38
    p->s.size += bp->s.size;
...
malloc(uint nbytes)
{
 ...
 639:	8b 45 08             	mov    0x8(%ebp),%eax
  if((prevp = freep) == 0){
 63c:	8b 3d 38 0a 00 00    	mov    0xa38,%edi
  nunits = (nbytes + sizeof(Header) - 1)/sizeof(Header) + 1;
 ...
 67f:	73 4f                	jae    6d0 <malloc+0xa0>
    ...
    if(p == freep)
 681:	8b 3d 38 0a 00 00    	mov    0xa38,%edi
 ...
 6aa:	e8 f1 fe ff ff       	call   5a0 <free>
  return freep;
 6af:	8b 15 38 0a 00 00    	mov    0xa38,%edx
 ...
 6f7:	0a 00 00 
    base.s.size = 0;
 6fa:	bf 3c 0a 00 00       	mov    $0xa3c,%edi
    base.s.ptr = freep = prevp = &base;
 ...
 709:	89 fa                	mov    %edi,%edx
    base.s.size = 0;
 70b:	c7 05 40 0a 00 00 00 	movl   $0x0,0xa40
 712:	00 00 00 
    if(p->s.size >= nunits){
